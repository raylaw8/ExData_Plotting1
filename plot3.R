...
#convert to date
date_time = as.POSIXlt(paste(sub_global_active_power$Date, sub_global_active_power$Time, sep=" "),format="%d/%m/%Y %H:%M:%S")
 
#plot line graph
plot(date_time,conv_sub_metering_1,type="l",xlab="", col="black", ylab="Energy sub metering")
lines(date_time,conv_sub_metering_2,type="l",xlab="", col="red", lty=1)
lines(date_time,conv_sub_metering_3,type="l",xlab="", col="blue", lty=1)
legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),col=c("red","black","blue"), lty=1)
